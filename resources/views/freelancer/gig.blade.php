...
<head>
  ...
  <title>WorkHub — Profile Setup (Upgraded)</title>
  ...
  <div class="min-h-screen flex items-start justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="w-full max-w-4xl">
      ...
      <header class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div>
            ...
            <div class="text-xl text-slate-500">Create a standout profile — make the first impression count</div>
          </div>
        </div>

        <div class="text-sm text-slate-500 hidden sm:flex items-center gap-4">
          <div class="text-xs">Need help?</div>
          <button class="px-3 py-2 text-sm rounded-md border border-slate-200 hover:bg-slate-50">Contact support</button>
        </div>
      </header>
      ...
      <main class="glass rounded-[14px] shadow-lg p-6 sm:p-8">
        ...
        <div class="mb-6">
          <div class="flex items-center justify-between">
            <div>
              <div class="text-lg font-medium text-slate-600">Profile setup</div>
              <div class="text-lg text-slate-400">2 steps • approx 3–5 minutes</div>
            </div>
          ...
          <div class="mt-3 w-full bg-slate-100 h-2 rounded-full overflow-hidden">
            <div id="progress" class="h-2 progress-fill bg-gradient-to-r from-sky-500 to-blue-700 w-1/4"></div>
          </div>

          <div class="mt-3 flex items-center gap-3 text-sm">
            <div class="flex items-center gap-2">
              <div id="dot-1" class="w-7 h-7 rounded-full flex items-center justify-center bg-white border border-slate-200 text-slate-700 font-medium">1</div>
              <div class="text-slate-600">Personal</div>
            </div>
            <div class="flex items-center gap-2">
              <div id="dot-2" class="w-7 h-7 rounded-full flex items-center justify-center bg-slate-50 border border-slate-200 text-slate-400 font-medium">2</div>
              <div class="text-slate-600">Professional</div>
            </div>
          </div>
        </div>

        <form id="profileForm" action="{{ route('gig.store')}}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-6">
          @csrf
          <section id="step-1" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
              ...
              <div class="sm:col-span-2">
                <input name="displayname" id="displayname" type="text"
                       ...
                       class="w-full p-3 rounded-lg border border-slate-200 focus-accent" />
                <div class="text-xs text-slate-400 mt-2">This is the name buyers will see prominently on your profile.</div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
              ...
              <div class="sm:col-span-2">
                <div class="flex items-center gap-4">
                  <div id="avatarPreview" class="w-28 h-28 rounded-xl overflow-hidden bg-slate-50 border border-slate-200 flex items-center justify-center text-3xl text-slate-400">
                    <span id="avatarInitial">D</span>
                  </div>

                  <div class="flex-1">
                    <label class="block">
                      <input name="profileimg" id="profileimg" type="file" accept="image/*" class="hidden" />
                      <button type="button" id="uploadBtn" class="px-4 py-2 btn-primary text-white rounded-md shadow-sm hover:opacity-95">Upload photo</button>
                    </label>
                    <div class="text-xs text-slate-400 mt-2">Square image, at least 400×400px recommended.</div>
                    <div class="text-xs text-slate-400 mt-1">Supported: JPG, PNG (max 5MB)</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
              <label class="text-sm text-slate-700 pt-2">Short bio <span class="text-red-500">*</span></label>
              <div class="sm:col-span-2">
                <textarea name="description" id="description" rows="5"
                          placeholder="Tell a bit about your experience, area of expertise and notable projects."
                          class="w-full p-3 rounded-lg border border-slate-200 focus-accent resize-none"></textarea>

                <div class="mt-2 flex items-center justify-between text-xs text-slate-400">
                  <div id="desc-min">Minimum 150 characters</div>
                  <div><span id="desc-count">0</span>/600</div>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-3 pt-2">
              ...
              <button type="button" id="to-step-2"
                      class="px-4 py-2 rounded-md btn-primary text-white shadow hover:opacity-95">
                Continue
              </button>
            </div>
          ...
          <section id="step-2" class="hidden space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
              <label class="text-sm text-slate-700 pt-2">Skills & experience <span class="text-red-500">*</span></label>

              <div class="sm:col-span-2">
                <div class="flex gap-2">
                  <select id="skillInput" class="p-3 rounded-lg border border-slate-200 focus-accent flex-1">
                  ...
                  <select id="skillExp" class="p-3 rounded-lg border border-slate-200 focus-accent">
                    <option value="Beginner">Beginner</option>
                    <option value="Intermediate" selected>Intermediate</option>
                    <option value="Expert">Expert</option>
                  </select>

                  <button id="addSkillBtn" type="button" class="px-4 py-2 rounded-md btn-primary text-white shadow">
                    Add
                  </button>
                </div>

                <div id="skillTags" class="mt-4 flex flex-wrap gap-2"></div>

                <div class="text-xs text-slate-400 mt-2">Add up to 12 skills. Each skill can have its experience level.</div>
              </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
              <label class="text-sm text-slate-700 pt-2">Education</label>
              <div class="sm:col-span-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
              <label class="text-sm text-slate-700 pt-2">Linked accounts</label>
            ...
            <div class="flex items-center justify-between gap-3 pt-2">
              <div class="text-sm text-slate-500">Review and submit your profile.</div>

              <div class="flex items-center gap-3">
                <button type="button" id="back-to-1"
                        class="px-4 py-2 rounded-md border border-slate-200 bg-white text-slate-700">
                  Back
                </button>

                <button type="submit" id="submitBtn"
                        class="px-4 py-2 rounded-md btn-primary text-white shadow">
                  Publish profile
                </button>
              </div>
            </div>
      ...
      <footer class="mt-5 text-center text-xs text-slate-400">
        By publishing you agree to <span class="text-slate-500">WorkHub Terms</span> · Preview: <span id="previewName" class="font-medium">—</span>
      </footer>
    </div>
  </div>
